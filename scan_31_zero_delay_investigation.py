...
ax.plot(T,i0_0H)
ax.legend(['1H','2H','3H','4H','5H','sum 1-5','sum 0-5','0H'])
...
for dev in device:
    ...
    for d in demod:
        ...
        Rh = np.append(Rh,R,axis=0)
        legend =np.append(legend,i)
        Phih =  np.append(Phih,Phi)
        
Rh = Rh.reshape((len(device)*len(demod),-1)).transpose()

        
figTD = plt.figure('R of demodulated signal')
order = [0,1,5,4,3] #defines in which order plots are made
Rh_sum =np.sum(Rh[::,order],axis=1)
ax = figTD.add_subplot(111)
ax.set_title('R of demodulated signal')
ax.plot(T, Rh[::,order])
ax.plot(T,Rh_sum)
ax.legend(np.append(legend[order],'sum 1-5'))
ax.set_xlabel('delay [fs]')
ax.set_ylabel('intensity [a.u.]')

#==============================================================================
# comparison of the sums of demodulated signal and of i0 FFT
#==============================================================================
