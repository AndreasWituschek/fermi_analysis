...
"""File I/O"""
path = './masterData.csv' #file path of masterData.csv:
data = fk.master_file_reader(path) #importin masterData.csv as dictionary
...
#plt.figure()
fk.plot_curve(Xtd, Ytd, start, stop)
fk.plot_td_data(data, demod)
...
if downshifted: 
    fk.plot_fd_curve(stop, start, cdft, cdft_d, l_ref, l_He, h)

#plot absorption spectrum in eV
if absolute:
    fk.plot_fd_curve_abs_ev(stop, start, cdft, cdft_d, l_ref, l_He, h)

#plot dispersive and absorptive part of spectrum
if absorptive:
    fk.plot_fd_curve_ev(stop, start, cdft, cdft_d, l_ref, l_He, h)
