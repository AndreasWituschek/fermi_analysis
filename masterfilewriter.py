...
delay_zero_pos= -19.8 #position of delay stage in mm where we have temporal overlap between seed pulses
...
ignore=1 #number of bunches to ignore after filter incident (depends on MFLI time constant)
I0_threshold= np.mean(I0)-3*np.std(I0) #lower threshold for I0 power filtering
modfreq_set = 1000. # modulation frequency in Hz set by the user at the AD9959 board
modfreq_threshold= .9 #[Hz] Disregards data when not within modfreq_set +- modfreq_threshold because then MFLI was not properly locked
...
mX,mY,sX,sY=fk.AveragingMfliData(mfli_data,I0,apply_filter,I0_threshold,modfreq_set,modfreq_threshold,ignore)
delay= (delay_pos - delay_zero_pos)*(ri-1)*np.sin(7./180*np.pi)/0.000299792458 #convert position [mm] of delay line to fs
#delay=4
