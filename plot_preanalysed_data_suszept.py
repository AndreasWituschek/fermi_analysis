...
title = 'Comparison'
color = 'r'
data_window = [0, 600] #[-200,370]
gauss = True # gauss window on TD (true) or not (False)
...
zeroPaddingFactor = 2
suscept = True
t_corr = 0  # correction on time zero
phi_sus = 0.0  # phase correction of data
...
#        if draw_theory:
        ...
        Xtd_gaus,Ytd_gaus,Xt_gaus,Yt_gaus = fk.Curve(l_fel/harmonic, l_ref, harmonic, phi, a*max(abs(Z)), offset, T[0], T[-1], 1000)
        Xt -= Xt_gaus*sp.exp(-((Ttheo)/(2*100))**2)
        Yt -= Yt_gaus*sp.exp(-((Ttheo)/(2*100))**2)
...
#        ax.errorbar(T, R, yerr=R_s, color=color, linestyle='')
        ...
        #ax.errorbar(delay, Phi, yerr=R_s, color='k', linestyle='')
        ax.plot(T, np.abs(np.append(np.diff(T)[0], np.diff(T))), '-', color=color)
        ...
        axs.plot(wn, dft.real, '-', color=color, linewidth = 2)
