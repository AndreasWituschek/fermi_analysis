...
%%%%Wavelenght of laser%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% WDp = 780.24; % Pulse carrier freq., in nm
WDp = 52.24; % Seed laser carrier freq., in nm for He 1s4p line
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
TDp = 99.0; %Pulse duration (FWHM of intensity) in fs, for transform limited pulse

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Discretization steps and time propagatios 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
max_tau12=20; % number of points along tau_12
step_tau = 0.02; % step size along tau in fs
...
% =23fs
WWp  = sqrt(8*log(2))/TDp * 5; % domain of the integration for the conversion of the pulse envelop into the time domain.  
...
Mu21R =   10.6586; % from ground S1/2 ---> P3/2
...
for itau12 = 1:max_tau12 %start of tau loop
    ...
    maxJ = round(ttt/Delta_t); %? maximum number of steps for integration of Schroedinger equation?
    ...
    WDpw = 2.*pi*speed_of_light/(WDp*10^-9)*1e-15;
    
    for jt2 = 1:maxJ2
        tt2 = tau0 + (jt2-1.)*Delta_t/2.0;
        Pulse1(jt2,1) = exp(-2*log(2)*((tt2-tau1)/TDp)^2)*exp(1i*WDpw*(tt2-tau1)); %definition of pulse directly in TD with fast oscillating field
        Pulse2(jt2,1) = exp(-2*log(2)*((tt2-tau2)/TDp)^2)*exp(1i*WDpw*(tt2-tau2));
%         Pulse1(jt2,1) = exp(-2*log(2)*((tt2-tau1)/TDp)^2); %definition of
%         pulse directly in TD without fast oscillating field
%         Pulse2(jt2,1) = exp(-2*log(2)*((tt2-tau2)/TDp)^2);
    ...
    %PuT = max(real(PulseT)); %delete?
    %PulseT = PulseT/PuT; %delete?
                
    %Pulse1real = real(Pulse1);
    %Pulse1imag = imag(Pulse1);

    %PulseWreal = real(PulseW);
    %PulseWimag = imag(PulseW);
...
%     if ((itau12==1))
%         FileName_pulse = [];
%         FileName_pulse = [BaseName, '_pulse'];
%         FileName_pulse = [FileName_pulse, '.mat'];
...
%                 
    ...
    for jt = 1:maxJ % start of jt loop
    	...
    	g1=F1*Pulse1(2*jt-1,1);
    	g2=F2*Pulse2(2*jt-1,1);
...
% figure('Name','Pulse1');
% plot(linspace(1,ttt,maxJ2),real(Pulse1))
% 
% figure('Name','Pulse2');
% plot(linspace(1,ttt,maxJ2),real(Pulse2))
